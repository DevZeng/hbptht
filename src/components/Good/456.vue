    ...
    <el-col :span="24" class="warp-main">
      ...
      >
        ...
        <el-form-item label="配送方式：" prop="delivery">
          <template>
            <el-radio-group v-model="newone.delivery" @change="changeRadio">
              <el-radio label="1">自提</el-radio>
              <el-radio label="2">快递</el-radio>
              <el-radio label="3">自提+快递</el-radio>
            </el-radio-group>
          </template>
        </el-form-item>

        <el-form-item label="选择自提地址" prop="self" v-show="selfshow">
          <el-table  :data="newone.self" ref="selftable" style="width: 50%" border size="mini" stripe  @selection-change="addselfid">
            <el-table-column type="selection" width="55" prop="sid" label="sid" ></el-table-column>
            <el-table-column prop="id" label="编号" width="55" ></el-table-column>
            <el-table-column prop="store_name" label="门店名称" min-width="150" align="center"></el-table-column>
          </el-table>
        </el-form-item>
        

        <el-form-item label="佣金：" prop="ratio">
          <el-input v-model="newone.ratio" type="number" placeholder="请输入佣金" style="width:500px;">
...
<script>
import { typeGet, comGetAll } from "../../api/api";
...
export default {
  ...
  methods: {
    ...
    checkid() {
      ...
      if (groupid) {
        ...
        groupGet(allParams).then(res => {
          ...
          this.newone.self=Object.values(res.data.self);       
            this.selfshow=true
            this.checkboxed(this.newone.self);
    ...
    checkboxed(rows){
      this.$refs["selftable"].toggleRowSelection(rows,true);
      if(rows){
